...
class TemporalEtasEstimate(Base):
    """Append-only temporal ETAS fit. Always cites the specific
    CompletenessEstimate whose Mc it used (same discipline as
    GutenbergRichterEstimate); optionally cites a
    ModifiedOmoriSequenceEstimate whose (K, c, p) seeded the optimizer's
    starting point, per docs/PROJECT_STATE.md's guidance that ETAS should
    build on the Modified Omori baseline rather than fit blind -- this is a
    documented starting-point reference, not a hard dependency, since ETAS
    is fit directly on the catalog and does not require declustering to
    run.
    """

    __tablename__ = "temporal_etas_estimates"
    __table_args__ = (CheckConstraint("event_count >= 0", name="event_count_non_negative"),)
